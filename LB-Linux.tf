...
# Network security group with HTTP allowed
...
resource "azurerm_virtual_machine_extension" "vm1" {
  ...
  settings = <<SETTINGS
    {
        "commandToExecute": "apt-get update && apt-get install -y apache2 && echo 'Hello from VM1' > /var/www/html/index.html"
    }
SETTINGS
}

resource "azurerm_virtual_machine_extension" "vm2" {
  name                 = "vm2-apache-install"
  ...
  settings = <<SETTINGS
    {
        "commandToExecute": "apt-get update && apt-get install -y apache2 && echo 'Hello from VM2' > /var/www/html/index.html"
    }
SETTINGS
}
